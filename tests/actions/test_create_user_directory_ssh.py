...
@pytest.mark.asyncio
async def test_create(keycloak_bootstrap, tmp_path, patch_ssh):
    attrs = {
        'uidNumber': 12345,
        'gidNumber': 12345,
    }
    ...
    exec(patch_ssh.call_args.args[1])

    ret_path = tmp_path / 'testuser'
    assert ret_path.is_dir()

@pytest.mark.asyncio
async def test_missing_uidNumber(keycloak_bootstrap, tmp_path, patch_ssh):
    attrs = {
        'gidNumber': 12345,
    }
    await users.create_user('testuser', first_name='first', last_name='last', email='foo@test', attribs=attrs, rest_client=keycloak_bootstrap)
...
@pytest.mark.asyncio
async def test_missing_gidNumber(keycloak_bootstrap, tmp_path, patch_ssh):
    attrs = {
        'uidNumber': 12345,
    }
    await users.create_user('testuser', first_name='first', last_name='last', email='foo@test', attribs=attrs, rest_client=keycloak_bootstrap)
...
@pytest.mark.asyncio
async def test_missing_group(keycloak_bootstrap, tmp_path, patch_ssh):
    attrs = {
        'gidNumber': 12345,
        'uidNumber': 12345,
    }
...
@pytest.mark.asyncio
async def test_already_exists(keycloak_bootstrap, tmp_path, patch_ssh):
