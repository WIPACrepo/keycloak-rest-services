...
(link:Ish1u6Xo) and defines the "calendar_id" (link:quaZ0hc1) attribute.
...
CALENDAR_ADDED = """
{addr} has been subscribed to the following calendar(s) as {role}:
...
CALENDAR_ROLE_CHANGED = """
{addr}'s role has changed from {old_role} to {new_role} for the following calendar(s):
...
async def sync_gws_calendars(calendar_acl, keycloak, creds, dryrun, notify):
    ...
    for kc_cal_group in kc_cal_groups:
        ...
        for missing_addr in set(target_rules) - set(actual_rules):
            ...
            if dryrun:
                continue
            logger.info(f"Inserting ACL {target_rules[missing_addr]}")
            ...
            logger.info("Forcing display of calendar")
...
def main():
    ...
    creds = service_account.Credentials.from_service_account_file(
        args['sa_credentials'], scopes=['https://www.googleapis.com/auth/calendar'])
    ...
    asyncio.run(sync_gws_calendars(calendar_acl_res, keycloak_client, creds,
