...
REMOVAL_PENDING_INSTRUCTIONS_ATTR = 'sync_composite_groups_member_removal_pending_message'
REMOVAL_AVERTED_INSTRUCTIONS_ATTR = 'sync_composite_groups_member_removal_averted_message'
REMOVAL_OCCURRED_INSTRUCTIONS_ATTR = 'sync_composite_groups_member_removal_occurred_message'
WELCOME_INSTRUCTIONS_ATTR = 'sync_composite_groups_member_welcome_message'
...
async def manual_sync(target_path: str,
    ...
    if attrs.get(IS_ENABLED_ATTR):
        ...
        return 1
    if group_cons_expr := attrs.get(CONSTITUENTS_EXPR_ATTR):
...
async def sync_composite_group(target_path: str,
    ...
    removal_pending_instructions = opts.get(REMOVAL_PENDING_INSTRUCTIONS_ATTR, '').replace('@@', '\n')
    removal_averted_instructions = opts.get(REMOVAL_AVERTED_INSTRUCTIONS_ATTR, '').replace('@@', '\n')
    removal_occurred_instructions = opts.get(REMOVAL_OCCURRED_INSTRUCTIONS_ATTR, '').replace('@@', '\n')
    welcome_instructions = opts.get(WELCOME_INSTRUCTIONS_ATTR, '').replace('@@', '\n')
    ...
    for current_member in current_members & intended_members:
        user = await _user_info(current_member)
        attrs = user['attributes']
        if attrs.get(removal_scheduled_user_attr_name):
            logger.info(f"Clearing {current_member}'s {removal_scheduled_user_attr_name} "
                        f"attribute ({dryrun, notify = })")
            if dryrun:
                continue
            await _modify_user(current_member, attribs={removal_scheduled_user_attr_name: None})
            if not notify or not removal_averted_instructions:
                continue
            address = notification_redirect_addr or f"{user['username']}@icecube.wisc.edu"
            ...
            send_email(address, f"You are no longer scheduled for removal from {target_path}",
                       REMOVAL_AVERTED_TEMPLATE.format(
                           ...
                           custom_text=removal_averted_instructions),
                       cc=notification_email_cc)

    # Process current members that don't belong to any constituent group,
    ...
    for extraneous_member in current_members - intended_members:
        ...
        if removal_grace_days:
            if removal_scheduled_at_str := attrs.get(removal_scheduled_user_attr_name):
                ...
                # "Removal scheduled attribute" attribute not set.
                # Set it and move on.
                ...
                send_email(address, f"You are scheduled for removal from {target_path}",
                           REMOVAL_AVERTED_TEMPLATE.format(
                               ...
                               group_path=target_path,
                               custom_text=removal_pending_instructions),
                           cc=notification_email_cc)
                continue
        # Either grace period not configured or grace period expired.
        # Remove the user from the group.
        logger.info(f"Clearing {extraneous_member}'s {removal_scheduled_user_attr_name} "
                    f"attribute ({dryrun, notify = })")
        ...
        if not notify or not removal_occurred_instructions:
            continue
        ...
        send_email(address, f"You have been removed from {target_path}",
                   REMOVAL_OCCURRED_TEMPLATE.format(
                       username=extraneous_member,
                       group_path=target_path,
                       custom_text=removal_occurred_instructions),
                   cc=notification_email_cc)
    ...
    for missing_member in intended_members - current_members:
        ...
        if not notify or not welcome_instructions:
            continue
        ...
        send_email(address, f"You have been added to {target_path}",
                   WELCOME_TEMPLATE.format(
                       ...
                       group_path=target_path,
                       custom_text=welcome_instructions),
                   cc=notification_email_cc)
